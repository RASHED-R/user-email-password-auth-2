...
const Register = () => {
    ...
    const handleRegister = e => {
        ...
        //every time we need to clear this warning, otherwise it shows on the display
        setSubmitSuccess('')
        setSubmitError('')

        // password validation
        if (password.length < 8) {
            setSubmitError('Password should be not less then');
            return;
        } else if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@#$%^&+=!]).{8,}$/.test(password)) {
            setSubmitError('Password must be at least 8 characters long, contain an uppercase letter, a lowercase letter, a digit, and a special character.')
            return
        } else if (!checked) {
    ...
    return (
        <div className=' flex justify-center'>
            <div className="relative flex flex-col text-gray-700 bg-transparent shadow-none rounded-xl bg-clip-border">
                ...
                <form className="max-w-screen-lg mt-8 mb-2 w-80 sm:w-96" onSubmit={handleRegister}>
                    <div className="flex flex-col gap-6 mb-1">
                        ...
                        <div className="relative h-11 w-full min-w-[200px]">
                            ...
                            <input type={showPassword ? "text" : "password"} onChange={handleOnChange} placeholder="********"
                                name='password'
                                required
                                className="peer h-full w-full rounded-md border border-blue-gray-200 border-t-transparent !border-t-blue-gray-200 bg-transparent px-3 py-3 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:!border-t-gray-900 focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50" />
                            <label
                    ...
                    {
                        submitError && <p className=" bg-red-500 p-2 text-white">{submitError}</p>
                    }
                    {
                        submitSuccess && <p className=" bg-green-500 p-2 text-white">User added Successfully</p>
                    }
                    <button
